...
# The remote user identity. Be default it should always be ec2-user if deployed on AWS EC2 instance
...
LOCAL_TOMCAT_BIN_DIR=/catamaran/servers/sl-india-erm/apache-tomcat-6.0.35/bin/
...
LOCAL_TOMCAT_WEBAPPS_DIR=/catamaran/servers/sl-india-erm/apache-tomcat-6.0.35/webapps
...
REMOTE_TOMCAT_WEBAPPS_DIR=/var/lib/tomcat6/webapps

# Location where the remote static site resides which should be served off of Apache directly
REMOTE_APACHE_WEBAPPS_DIR=/var/www/html

# Location where the local static resources reside which should be served off of Apache
LOCAL_APACHE_WEBAPPS_DIR=/projects/madakethealth-corporatesite

# the name of your local webapp war file for the apache resources
APACHE_WEBAPP_NAME=madaket-corporate
...
if [ "$COMMAND" == "build" ]; then
  ...
  if [ ! $ERROR_STATUS -eq 0 ]; then
    echo "cd command failed, check path: $LOCAL_WEBAPP_DIR/WEB-INF/"
    exit 1
  fi    
...
if [ "$COMMAND" == "deploy" ]; then
  ...
  # Then build a war (and timestamp its build.ftl file)
...
if [ "$COMMAND" == "deploy-corp" ]; then
  ...
  # Build a war 
  echo "CATAMARAN Running 'mvn clean install' .."
  cd $LOCAL_APACHE_WEBAPPS_DIR
  mvn clean install

  # Copy to remote
  echo "CATAMARAN Copying war file to $TARGET_SERVER"
  rsync -e ssh -rcav --delete-after \
    $LOCAL_APACHE_WEBAPPS_DIR/target/$APACHE_WEBAPP_NAME/* \
    ${REMOTE_USER}@${TARGET_SERVER}:${REMOTE_APACHE_WEBAPPS_DIR}
